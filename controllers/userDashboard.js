...
const { processAnswers, saveTestSubmission, saveTestResults, buildSuccessResponse } = require("../utils/testGrading")
...
const userDashboardController = {
    ...
    courseList: async (req, res) => {
        try {
            ...
            res.status(200).json({ success: true, message: "Course fetched", courses: userCourses, count: courseCount });
        } catch (error) {
    ...
    userCourses: async (req, res) => {
        try {
            ...
            if (!user) {
                return res.status(404).json({ success: false, message: "success:false,User not found" });
            }
    ...
    userCourseById: async (req, res) => {
        try {
            ...
            if (!course) {
                return res
                    .status(403)
                    .json({ success: false, message: "You are not registered for this course" });
            }

            res.status(200).json({ success: true, message: "User course found", course });
        } catch (error) {
    ...
    viewUserTest: async (req, res) => {
        try {
            ...
            if (
                ...
                return res.status(400).json({ success: false, message: "Invalid test or user ID" });
            }
    ...
    startUserTest: async (req, res) => {
        try {
            ...
            if (!test) {
                return res.status(403).json({ success: false, message: 'Test not found or not accessible for your courses' });
            }

...
async function gradeWithAI(question, answer, maxPoints) {
    ...
    console.error({ success: false, lastError, message: 'AI grading failed after retries:' });
    return null;
}
    ...
    try {
        ...
        console.log({
            ...
            conceptsAddressed: Array.isArray(result.concepts) ?
                result.concepts : [result.concepts].filter(Boolean),
            improvementAreas: Array.isArray(result.improvements) ?
                result.improvements : [result.improvements].filter(Boolean),
...
function basicTheoryGrading(question, answer, maxPoints) {
    ...
    const structureScore = (hasParagraphs && hasExamples) ? 0.2 :
        (hasParagraphs || hasExamples) ? 0.1 : 0;

    const totalScore = Math.round((lengthScore + keywordScore + structureScore) * maxPoints);

    // Generate feedback
    const feedbackParts = [];
    feedbackParts.push(wordCount >= MIN_WORDS ?
        `Met minimum length (${wordCount} words)` :
        `Too short (${wordCount}/${MIN_WORDS} words)`);

    feedbackParts.push(`Matched ${matchedKeywords.length}/${KEYWORDS.length} keywords`);

    if (!hasParagraphs) feedbackParts.push('Consider using paragraphs');
    if (!hasExamples) feedbackParts.push('Add specific examples');
    ...
    console.log({
        ...
        improvementAreas: [
            ...KEYWORDS.filter(kw => !matchedKeywords.includes(kw))
                .map(kw => `Include "${kw}"`),

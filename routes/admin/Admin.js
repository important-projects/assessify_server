...
} = require("../../../../assessifyRevamped/server/utils/mediaUpload");
// extractQuestionsFromPdf
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ message: "Error checking course", error: err.message });
    }
    ...
    try {
      ...
      if (file) {
        ...
        if (!extractedQuestions.length) {
          ...
          return res
            .status(400)
            .json({ message: "No questions found in file" });
        }
...
router.get("/results", protect, isAdmin, async (_, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error retrieving scores", error: err.message });
  }
});
...
router.get("/top10", protect, isAdmin, async (_, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error retrieving top scores", error: err.message });
  }
});
...
router.get("/pieData", protect, isAdmin, async (_, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error retrieving pie chart data", error: err.message });
  }
});
...
router.get("/leaderboard", protect, isAdmin, async (_, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error retrieving leaderboard", error: err.message });
  }
});
